...
const addSize = (size) => {
  const currPath = [...fd['$$currPath']] // duplicate currPath for editing
  const addIt = () => {
    const pathRef = currPath.reduce((x,y)=>x[y], fd) // makes path reference for file system object
    pathRef.$size += size // actual size adding
    addTotals(pathRef)
  ...
  const addTotals = (pathRef) => {
    let pathStr = currPath.join('')
    if (pathStr !== '') {
      fd.$$dirTotals[pathStr] = pathRef.$size
    }
  }
  addIt()
}
...
const addData = (name, size = -1) => { // datatype 0 = dir, datatype 1 = file
  ...
  if (size > -1) { // make new file
    pathRef[newName] = size
    addSize(size)
  } else { // make new directory
    pathRef[newName] = {
      $size: 0
    }
    fd.$$dirTotals[`${fd.$$currPath.join('')}${newName}`] = 0 // add directory to fd.$$dirTotals for size tracking
  }
}

const parser = (cmd) => {
  const tasks = cmd.split(" ")
  if (tasks[0] == "$") { // is it a command?
    switch(tasks[1]) {
      case "cd":
        switch(tasks[2]) {
          case "/":
            fd.$$currPath.length = 0 // go to root
            break
          case "..":
            fd.$$currPath.pop() // go back a directory
            break
          default:
            fd.$$currPath.push(tasks[2]) // jump into a directory
        }
        break
      case "ls": // list directories, nothing changes
        break
    }
  } else if (tasks[0] == "dir") { // create a directory
    addData(tasks[1])
  } else { // create a file
    addData(tasks[1], Number(tasks[0]))
  }
}
...
const answer1 = () => {
  ...
  for (let dir in fd.$$dirTotals) {
    if (fd.$$dirTotals[dir] <= 100000) {
      filteredTotals += fd.$$dirTotals[dir]
    }
  }
...
const answer2 = () => {
  ...
  let usedSpace = 0
  Object.keys(fd).forEach(x=>{
    if (x[0] !== '$') {
      if (typeof fd[x] == 'number') {
        usedSpace += fd[x]
      } else if (typeof fd[x].$size == 'number') {
        usedSpace += fd[x].$size
      }
    }
  })
  ...
  const candidateDirs = []
  for (let dir in fd.$$dirTotals) {
    let dirTotal = fd.$$dirTotals[dir]
    if (dirTotal >= (neededSpace - freeSpace)) {
      candidateDirs.push(dirTotal)
    }
  }
  candidateDirs.sort((a,b)=>a - b)
